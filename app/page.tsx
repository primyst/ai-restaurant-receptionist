...
import { findMenuMatches, findUnavailableWords, menu, type MenuItem } from '../lib/menu';
...
export default function Home() {
  ...
  const submit = (e?: FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (matches.length) {
        ...
        const requested = unavailable.length ? unavailable.join(', ') : 'that item';
        ai(`I’m sorry, we don’t currently have ${requested} on the menu. I can only add items from the restaurant’s current catalog. You can ask for something like jollof, fried rice, chicken, suya, plantain, egusi, Coke, Sprite or water.`);
      }
    }, 450);
  };
  ...
  const detailsReady = Boolean(name.trim() && phone.trim() && (delivery === 'pickup' || address.trim()));
  ...
  const pay = () => { setStage('tracking'); setStatus(1); ai('Payment confirmed. Your order is now with the kitchen. I’ll keep you posted here.'); setTimeout(() => setStatus(2), 1800); setTimeout(() => setStatus(3), 4200); };
